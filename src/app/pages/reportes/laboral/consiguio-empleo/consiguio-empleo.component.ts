...
@Component({
  selector: 'app-consiguio-empleo',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './consiguio-empleo.component.html',
  styleUrl: './consiguio-empleo.component.css'
})
export class ConsiguioEmpleoComponent implements OnInit, OnDestroy {
  ...
  construir() {
    ...
    const labels = ['Bolsa de trabajo','Internet','Recomendacion','Entrevista'];
    const values = labels.map(l => datos.filter(d => d.comoConsiguio === l).length);
    const total = values.reduce((a,b) => a+b, 0);
    const canvas = document.getElementById('chart') as HTMLCanvasElement;
    if (!canvas) return;
    this.chart = new Chart(canvas, {
      type: 'doughnut',
      data: { labels, datasets: [{ data: values, backgroundColor: ['#2f8f83','#52b0a4','#85cdc6','#1a6e78'] }] },
      options: {
