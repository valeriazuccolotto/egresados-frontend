...
export class NivelComponent implements OnInit, OnDestroy {
  ...
  construir() {
    ...
    const labels = ['Maestria','Doctorado'];
    const values = labels.map(l => datos.filter(d => d.nivelEstudio === l).length);
    const total = values.reduce((a,b) => a+b, 0);
    const canvas = document.getElementById('chart') as HTMLCanvasElement;
    if (!canvas) return;
    this.chart = new Chart(canvas, {
      type: 'pie',
      data: { labels, datasets: [{ data: values, backgroundColor: ['#2f8f83','#1a6e78'] }] },
      options: {
