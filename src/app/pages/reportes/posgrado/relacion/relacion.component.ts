...
export class RelacionComponent implements OnInit, OnDestroy {
  ...
  construir() {
    ...
    const labels = ['Si','No','Un poco'];
    const values = labels.map(l => datos.filter(d => d.relacionadoCarrera === l).length);
    const total = values.reduce((a,b) => a+b, 0);
    const canvas = document.getElementById('chart') as HTMLCanvasElement;
    if (!canvas) return;
    this.chart = new Chart(canvas, {
      type: 'doughnut',
      data: { labels, datasets: [{ data: values, backgroundColor: ['#2f8f83','#9eaab3','#85cdc6'] }] },
      options: {
