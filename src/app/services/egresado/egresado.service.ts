...
export class EgresadoService {
  ...
  private url = 'http://localhost:8189/egresados';
  ...
  getLaboralPorMatricula(matricula: string): Observable<any[]> {
    return this.http.get<any[]>(`http://localhost:8189/egresado/laboral/${matricula}`);
  }

  getPosgradoPorMatricula(matricula: string): Observable<any[]> {
    return this.http.get<any[]>(`http://localhost:8189/egresado/posgrado/${matricula}`);
  }

  getReconocimientosPorMatricula(matricula: string): Observable<any[]> {
    return this.http.get<any[]>(`http://localhost:8189/egresado/reconocimientos/${matricula}`);
  }

  getCertificacionesPorMatricula(matricula: string): Observable<any[]> {
    return this.http.get<any[]>(`http://localhost:8189/egresado/certificaciones/${matricula}`);
  }
